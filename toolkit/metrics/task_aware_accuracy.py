...
class AccuracyPluginMetric(GenericPluginMetric[float]):
    ...
    def update(self, strategy):
        ...
        # Modify mb_y and output to match in shape of the number of class in the task
        num_classes = len(np.unique(strategy.experience.dataset.targets))

        # Its a bit dangerous to use this way of having classes in experience
        mb_y = strategy.mb_y - torch.ones_like(strategy.mb_y)*current_experience*num_classes
        output = strategy.mb_output[:, current_experience*num_classes:(current_experience+1)*num_classes]

        self._accuracy.update(output, mb_y, current_experience)
...
class StreamAccuracy(AccuracyPluginMetric):
    ...
    def __str__(self):
        return "Top1_TAW_Stream"


def task_aware_accuracy_metrics(
    *,
    minibatch=False,
    epoch=False,
    stream=False,
