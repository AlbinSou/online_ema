...
class AccuracyPluginMetric(GenericPluginMetric[float]):
    ...
    def update(self, strategy):
        # Here even in online setting, current experience should be from val or test stream so
        # It will return the true one
        ...
        self._accuracy.update(strategy.mb_output, strategy.mb_y, current_experience)
...
class StreamAccuracy(AccuracyPluginMetric):
    ...
    def __str__(self):
        return "Top1_ExpAcc_Stream"


def per_experience_accuracy_metrics(
    *,
    minibatch=False,
    epoch=False,
    stream=False,
